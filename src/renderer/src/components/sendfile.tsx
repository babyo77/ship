...
const Sendfile: React.ForwardRefRenderFunction<HTMLButtonElement, SendfileProps> = (
  { sentFile, setSentFile },
  ref
) => {
  ...
  const handleOnDrop = useCallback(
    (e: React.DragEvent<HTMLDivElement>) => {
      try {
        ...
        setSentFile((prev) => [...filesToSend, ...prev])
        ...
        {sentFile.length === 0 ? (
          ...
          <div className="leading-tight font-normal text-zinc-400">
            ...
            <div className="bottom-0 left-0.5 cursor-pointer hover:text-zinc-200 transition-all duration-300 fixed py-2 px-2 items-center flex justify-center rounded-full">
              <p className="text-xs text-zinc-500">Sent {sentFile.length}</p>
              <p
                onClick={() => setSentFile([])}
...
export default forwardRef(Sendfile)
